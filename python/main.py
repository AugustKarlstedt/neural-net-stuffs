...
class neural_network:
  ...
  def __init__(self):
    self.hidden_layer_nodes = [2, 17, 9, 4]
    self.epochs = 10000
  ...
  def fit(self, x: list, y: list):
    ...
    for epoch in range(self.epochs):
      ...
      for example_index in range(self.mini_batch_size):
        ...
        weight_deltas[-1] = weight_deltas[-1] + np.dot(delta, np.transpose(activations[-1]))
        bias_deltas[-1] = bias_deltas[-1] + delta

        for l in range(2, self.layer_count):
          delta = np.dot(np.transpose(self.weights[-l+1]), delta) * neural_network.sigmoid_prime(zs[-l])
          weight_deltas[-l] = weight_deltas[-l] + np.dot(delta, np.transpose(activations[-l]))
